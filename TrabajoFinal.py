...
st.title("Predicción del Departamento de Origen del Café")

# Entrada de datos del usuario
st.sidebar.header("Ingrese las características")
def user_input_features():
    variable1 = st.sidebar.number_input("Variable 1", value=0.0)
...
# Sección de predicción para cada modelo
st.subheader("Predicciones de los Modelos")

# LDA
st.markdown("### Modelo LDA")
lda_pred = lda_model.predict(input_df)
st.write(f"Predicción: {lda_pred[0]}")

# QDA
st.markdown("### Modelo QDA")
qda_pred = qda_model.predict(input_df)
st.write(f"Predicción: {qda_pred[0]}")

# Random Forest
st.markdown("### Modelo Random Forest")
rf_pred = rf_model.predict(input_df)
st.write(f"Predicción: {rf_pred[0]}")

# SVM
st.markdown("### Modelo SVM")
svm_pred = svm_model.predict(input_df)
st.write(f"Predicción: {svm_pred[0]}")
